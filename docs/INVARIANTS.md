...
| S8 | `revealMode: "single-word"` renders only the active word — others are absent from output, not hidden-but-present. `captionAnimation` (32 values, contract §8) is a cosmetic entrance transition, orthogonal to `revealMode`, no bounds check. Two values (`karaokeFill`/`karaokeBox`) are the sole exception — they change how a word's own highlight is drawn, not just how the block enters. | arch §6-7, contract §8 |
