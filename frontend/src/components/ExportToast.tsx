...
interface ExportToastProps {
  prefs: Prefs;
  strings: Strings;
  isDone: boolean; // false = failed
  onOpen: () => void;
  onDownload?: () => void; // omitted for the failed case -- nothing to download
  onDismiss: () => void;
}

// Ported from the design's e_exportToastStyle block: bottom-right corner card, shown only while
// the modal is minimized AND the job has actually finished (done or failed) -- a still-running
// minimized export has no persistent indicator of its own, only the ring on the Export button.
// No auto-dismiss timer anywhere in the design's own toast path, confirmed by the human this
// session -- only the ✕ closes it.
    ...
    >
      <div
        style={{
          ...
          height: "24px",
          borderRadius: "50%",
          background: isDone ? "#22c55e" : "#ef4444",
          display: "flex",
          alignItems: "center",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px", flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: "12.5px", fontWeight: 700, color: mode.textMain }}>
          {isDone ? L.exportDone : L.exportFailed}
        </div>
