...
async def _create_bare_project() -> uuid.UUID:
...
async def _create_fully_transcribed_project(sample_video: Path) -> uuid.UUID:
...
async def _delete(project_id: uuid.UUID) -> httpx.Response:
...
async def test_delete_a_bare_project_succeeds() -> None:
    ...
    project_id = await _create_bare_project()
