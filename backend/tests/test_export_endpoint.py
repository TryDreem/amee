...
# test_transcribe_endpoint.py: export_task's own asyncio.run() can't run
# nested inside the async route handler's event loop. `.delay()` publishes
# to the real broker and returns immediately without executing the task
# body; the actual rendering pipeline is exercised from plain sync test
...
async def _create_project(sample_video: Path) -> tuple[uuid.UUID, dict[str, str]]:
    async with async_session_factory() as session:
        ...
        # POST /export resolves the submitted style's presetId to validate
        # bounds (X5) - a project always has a style eagerly created at
        # upload in production (contract §4), so tests need one too.
...
async def test_export_returns_404_for_missing_project() -> None:
    async with httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        response = await client.post(
            f"/api/v1/projects/{uuid.uuid4()}/export", json=_valid_body()
        )
...
async def test_export_persists_ecs_and_style_before_enqueueing(
    ...
    assert ecs_response.status_code == 200
    assert ecs_response.json()["segments"][0]["words"][0]["text"] == "hi"
    assert style_response.status_code == 200
    assert style_response.json()["presetId"] == _DEFAULT_PRESET_ID
